"""
Space Voxel Viewer - Astronomical voxel-based visualization

This script provides visualization for astronomical data in a 3D voxel grid.
It processes FITS image files and projects them into a 3D voxel space, allowing
for locating objects in 3D space from multiple 2D observations.

Requirements:
    numpy, matplotlib, astropy, Open3D (optional)
"""
...
try:
    ...
    print("Warning: process_image_cpp module not found. Limited functionality available.")
...
def process_image(fits_file, voxel_grid, voxel_grid_extent, celestial_sphere_texture):
    ...
    with fits.open(fits_file) as hdulist:
        ...
        if 'process_image_cpp' in globals():
            ...
            print("Warning: C++ module not available. Skipping ray casting.")
...
def main():
    ...
    # List FITS files
    if os.path.exists(fits_directory):
        fits_files = [os.path.join(fits_directory, f) for f in os.listdir(fits_directory) if f.endswith('.fits')]
    else:
        print(f"Warning: FITS directory '{fits_directory}' not found. Creating an empty one.")
